...
| 7    | UI default flips to real backend (Phase 3)        | ✅ wired (env override) — `VITE_WS_URL` was added in Step 3 and works against the real backend at `ws://localhost:8080/v1/ws`. The default-flip is deferred until a staging deployment of `qua-media-rs` is healthy enough that the real backend is the better default for UI dev (today it needs Postgres + Kafka + S3 + ffmpeg + a seeded run). |
| 8    | Mock becomes conformance harness (Phase 4)        | ✅ done — [`client/tests/e2e/ws-schema-validation.spec.ts`](https://github.com/danielmgzzg/qua-media-ui/blob/main/client/tests/e2e/ws-schema-validation.spec.ts) captures every inbound WS frame and validates each against `schemas/v1/ws/server.schema.json` via Ajv 8 (draft-2020-12) with `domain.schema.json` registered for cross-file `$ref` resolution. Runs in CI as part of the Playwright e2e suite. |
