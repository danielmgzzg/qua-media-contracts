...
| 8    | Mock becomes conformance harness (Phase 4)        | 🟡 partial — shape-level contract assertions live in [`client/tests/e2e/ws-contract.spec.ts`](https://github.com/danielmgzzg/qua-media-ui/blob/main/client/tests/e2e/ws-contract.spec.ts) (snapshot within 5s, all 11 stages, continuous events, reconnect). Schema-driven validation against bundled `schemas/v1/ws/server.schema.json` (Ajv + cross-file `$ref`) is the natural completion. |
